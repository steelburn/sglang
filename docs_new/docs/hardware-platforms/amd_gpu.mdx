...
cd sgl-kernel
# Set your GPU architecture: gfx942 (MI300), gfx950 (MI350), or gfx1151 (RDNA3)
...
## RDNA 3 Consumer GPUs (gfx1151)

SGLang supports AMD RDNA 3 consumer GPUs (e.g., Radeon 8060S, gfx1151) with the following considerations:

- **Attention backend:** Use `--attention-backend torch_native` (FlashInfer is CUDA-only, AITER is CDNA-only).
- **CUDA graphs:** Use `--disable-cuda-graph`. CUDA graphs trigger a ROCr driver crash on gfx1151 (`rocr::core::InterceptQueue::Submit`).
- **Tool support:** Use `--tool-call-parser qwen` for Qwen models (supports function calling with `tool_choice: "auto"`).
- **No RDMA/Mooncake:** Consumer GPUs lack the necessary interconnects for disaggregated serving.
- **No MXFP4:** Requires CDNA3/CDNA4 hardware.
- **Context length:** 256K+ context models work; use `--max-total-tokens` to manage KV cache memory.

### Verified models

These models have been tested and verified on gfx1151:

| Model | Size | Architecture | Load Time | Inference |
|---|---|---|---|---|
| `Qwen/Qwen2.5-0.5B-Instruct` | 0.5B | Qwen2.5 | ~10s | ✅ |
| `Qwen/Qwen3.5-4B` | 4B | Qwen3.5 hybrid (24 MLA + 8 full attn) | ~27s | ✅ ~2.5s first token, tool calls ✅ |
| `Qwen/Qwen3.5-9B` | 9B | Qwen3.5 hybrid (24 MLA + 8 full attn) | ~35s | ✅ ~4.8s first token |

> **Note:** On gfx1151, use `--attention-backend torch_native` (FlashInfer is CUDA-only,
> AITER is CDNA-only). The triton attention backend may have compilation issues
> on RDNA3 with certain model architectures. Always pair with `--disable-cuda-graph`.
...
A dedicated Dockerfile for gfx1151 is available at `docker/gfx1151.Dockerfile`. It uses ROCm 7.2.4 base and includes the necessary patches for torchvision and sgl_kernel compatibility on RDNA 3.
