...
class _SkipCheckModel(nn.Module):
    """A normal param plus one flagged _skip_weight_check (mirrors kv-cache k_scale/v_scale)."""

    def __init__(self):
        super().__init__()
        self.w = nn.Parameter(torch.randn(4, 4), requires_grad=False)
        self.scale = nn.Parameter(torch.full((1,), 2.0), requires_grad=False)
        self.scale._skip_weight_check = True


class TestResetSkipsSkipWeightCheck(CustomTestCase):
    """reset_tensors must skip _skip_weight_check params: reset only poisons what a
    later compare verifies, and these are excluded from compare/checksum, so poisoning
    one checks nothing and could strand a sentinel as a real value."""

    def setUp(self):
        torch.manual_seed(0)
        self.model = _SkipCheckModel().cuda()
        self.checker = WeightChecker(model_runner=_FakeModelRunner(self.model))

    def test_reset_skips_skip_weight_check_param(self):
        before_scale = self.model.scale.clone()
        self.checker._reset_tensors()
        # Normal param is poisoned to the sentinel...
        torch.testing.assert_close(
            self.model.w, torch.full_like(self.model.w, _RESET_SENTINEL)
        )
        # ...but the _skip_weight_check param is left untouched.
        torch.testing.assert_close(self.model.scale, before_scale)


class _VisionLeaf(nn.Module):
    def __init__(self):
        super().__init__()
...
class _VisionModel(nn.Module):
    """An LLM-style param plus vision-encoder + projector submodules named like real VL
    models (`visual.` GLM/Qwen, `vision_tower.` Kimi/LLaVA, `mm_projector.` Kimi K2.5)."""

    def __init__(self):
        super().__init__()
        self.language_weight = nn.Parameter(torch.randn(4, 4), requires_grad=False)
        self.visual = _VisionLeaf()  # -> "visual.weight"
        self.vision_tower = _VisionLeaf()  # -> "vision_tower.weight"
        self.mm_projector = _VisionLeaf()  # -> "mm_projector.weight"


class TestVisionEncoderSkip(CustomTestCase):
    """include_visual=False loads vision weights normally but excludes them from
    reset and compare; include_visual=True checks them like any param."""

    def setUp(self):
        torch.manual_seed(0)
        self.model = _VisionModel().cuda()
        self.checker = WeightChecker(model_runner=_FakeModelRunner(self.model))

    def test_reset_skips_vision_when_excluded(self):
        before_visual = self.model.visual.weight.clone()
        before_tower = self.model.vision_tower.weight.clone()
        before_proj = self.model.mm_projector.weight.clone()
        self.checker._reset_tensors(include_visual=False)
        # LLM param is poisoned...
        torch.testing.assert_close(
            self.model.language_weight,
            torch.full_like(self.model.language_weight, _RESET_SENTINEL),
        )
        # ...vision encoder + projector weights are left untouched.
        torch.testing.assert_close(self.model.visual.weight, before_visual)
        torch.testing.assert_close(self.model.vision_tower.weight, before_tower)
        torch.testing.assert_close(self.model.mm_projector.weight, before_proj)

    def test_reset_poisons_vision_when_included(self):
        self.checker._reset_tensors(include_visual=True)
        torch.testing.assert_close(
            self.model.visual.weight,
            torch.full_like(self.model.visual.weight, _RESET_SENTINEL),
        )

    def test_compare_skips_vision_when_excluded_but_catches_it_when_included(self):
        self.checker._snapshot()
        with torch.no_grad():
            self.model.visual.weight.fill_(123.0)
        # include_visual=False: a diverged vision weight must not fail compare.
        self.checker._compare(include_visual=False)
        # include_visual=True: the same divergence is caught.
        with self.assertRaises(Exception) as ctx:
            self.checker._compare(include_visual=True)
        self.assertIn("name=visual.weight", str(ctx.exception))
